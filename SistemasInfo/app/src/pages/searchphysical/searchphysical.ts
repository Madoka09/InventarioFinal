...
export class SearchphysicalPage {
  ...
  scanCode() {
    ...
    loader.present().then(() => {
      ...
      this.http.post('http://172.10.20.194/IonicApp/qrcode_check.php', data, options)
      .map(res => res.json())
      .subscribe(res => {
      ...
      loader.dismiss();
        ...
        this.showToast('Producto Encontrado!');
  ...
  getData(){
    this.httpC.get("http://172.10.20.194/IonicApp/qrcode_check.php").subscribe(dataQ => {
      this.elements = dataQ;
      console.log(dataQ);
    }, err => {
      console.log(err);
